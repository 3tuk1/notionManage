...
def upload_file_to_notion(file_path, file_name, parent_page_id):
    """ファイルをNotionの指定ページにアップロードし、ファイル列を更新する"""
    try:
        # ファイルをバイナリモードで読み込み
        ...
        # Notionにファイルをアップロード
        url = f"{NOTION_API_URL}/blocks/{parent_page_id}/children"
        ...
        if response.status_code == 200:
            print(f"ファイルをNotionにアップロードしました: {file_name}")
            return True
        else:
            print(f"ファイルのアップロードに失敗しました: {response.status_code} {response.text}")
            return False
    except Exception as e:
        print(f"ファイルアップロード中にエラーが発生: {e}")
        return False

def download_and_upload_file_to_notion(file_url, file_name, parent_page_id):
    """
    NotionのファイルURLからファイルをダウンロードし、Notionにアップロードして永続リンクを取得
    """
    try:
        # ファイルをダウンロード
        ...
        # Notionにアップロード
        success = upload_file_to_notion(temp_path, file_name, parent_page_id)

        # 一時ファイルを削除
        os.unlink(temp_path)

        if success:
            print(f"Notionへのアップロード完了: {file_name}")
            return True
        else:
            print(f"Notionへのアップロードに失敗しました: {file_name}")
            return False
